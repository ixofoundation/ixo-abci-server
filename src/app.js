...
CounterApp.prototype.deliverTx = function (req, cb) {
  console.log('TX_DELIVER: ' + JSON.stringify(req, null, '\t'));
  ...
  if (this.serial) {
    ...
    if (txValue != this.txCount) {
      return cb({
        code: abci.CodeType.BadNonce,
        log: "Nonce is invalid. Got " + txValue + ", expected " + this.txCount
      });
    }
  }
  this.txCount += 1;
...
CounterApp.prototype.checkTx = function (req, cb) {
  console.log('TX_CHECK: ' + JSON.stringify(req, null, '\t'));
  ...
  if (this.serial) {
    ...
    if (txValue < this.txCount) {
      return cb({
        code: abci.CodeType.BadNonce,
        log: "Nonce is too low. Got " + txValue + ", expected >= " + this.txCount
      });
    }
  }
  return cb({code: abci.CodeType_OK});
...
CounterApp.prototype.commit = function (req, cb) {
  console.log('TX_COMMIT: ' + JSON.stringify(req, null, '\t'));
...
console.log("IXO Tendermint ABCI");
